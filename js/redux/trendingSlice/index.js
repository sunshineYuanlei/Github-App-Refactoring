...
// 下拉刷新
export const setRefreshTrendingAsync = createAsyncThunk(
  'trending/setRefreshTrendingAsync',
  async payload => {
    // payload: {storeName, url, pageSize, favoriteDao}
    const {storeName, url, pageSize, favoriteDao} = payload;
    let dataStore = new DataStore();
    const data = await dataStore.fetchData(url, FLAG_STORAGE.flag_trending);

    // console.log('response-setRefreshTrendingAsync:', data);
...
// 上拉加载更多
export const setLoadMoreTrendingAsync = createAsyncThunk(
  'trending/setLoadMoreTrendingAsync',
  async payload => {
...
const trendingSlice = createSlice({
  ...
  initialState: {},
  reducers: {
    setFlushTrendingFavorite(state, {payload}) {
      // payload: {storeName, pageIndex, pageSize, dataArray, favoriteDao}
  ...
  extraReducers: builder => {
    ...
    builder
      .addCase(setRefreshTrendingAsync.pending, (state, {meta}) => {
      ...
      .addCase(setRefreshTrendingAsync.fulfilled, (state, {payload}) => {
      ...
      .addCase(setRefreshTrendingAsync.rejected, (state, {payload}) => {
        state[payload.storeName] = {
          ...state[payload.storeName],
          isLoading: false,
        };
      })
      .addCase(setLoadMoreTrendingAsync.fulfilled, (state, {payload}) => {
        state[payload.storeName] = {
          ...state[payload.storeName],
      ...
      .addCase(setLoadMoreTrendingAsync.rejected, (state, {payload}) => {
        state[payload.storeName] = {
          ...state[payload.storeName],
          pageIndex: --payload.pageIndex,
          hideLoadingMore: false,
...
export const {setFlushTrendingFavorite} = trendingSlice.actions;
